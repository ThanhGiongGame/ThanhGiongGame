    ...
    public class EquipmentUI : MonoBehaviour
    {
        ...
        {
            Debug.Log("EquipmentUI OnEnable");

            if (FindFirstObjectByType<ShopUI>(FindObjectsInactive.Include) != null)
            {
                Debug.Log("Found ShopUI -> Disable EquipmentUI");
                enabled = false;
                return;
            }
        ...
        private void BuildEquipment()
        {
            ...
            {
                Debug.Log($"{item.id} Unlocked = {item.IsUnlocked()}");
            }
            foreach (var item in InventoryManager.Instance.allItems)
            {
        ...
        private void OnItemSelected(GameItemData item)
        {
            ...
            if (!item.IsUnlocked())
            {
                equipButtonText.text = "CẦN CẤP ĐỘ " + item.requiredLevel;
                equipButton.interactable = false;
            }
            else
            {
                equipButtonText.text = item.IsEquipped() ? "ĐANG TRANG BỊ" : "TRANG BỊ";
